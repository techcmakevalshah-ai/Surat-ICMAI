...
if (!url || !secret) throw new Error('Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SECRET_KEY first.')
...
function readWorkbook(filePath, source) {
  ...
  for (const row of mapped) deduped.set(`${row.source}|${row.registration_number}`, row)
  return Array.from(deduped.values())
}

const records = [
  ...readWorkbook(path.resolve(interFile), 'INTER'),
  ...readWorkbook(path.resolve(fndFile), 'FND'),
]

for (let i = 0; i < records.length; i += 200) {
  const batch = records.slice(i, i + 200)
  const { error } = await supabase.from('students').upsert(batch, { onConflict: 'source,registration_number' })
  if (error) throw error
  console.log(`Imported ${Math.min(i + batch.length, records.length)} / ${records.length}`)
}

console.log(`Done. ${records.length} unique student records imported.`)
